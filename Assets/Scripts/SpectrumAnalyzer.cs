...
public class SpectrumAnalyzer : MonoBehaviour
{
    ...
    private List<GameObject> bulbs; //ref pillars to scale/move with music
    ...
    private void CreatePillarsByShapes()
    {
        ...
        bulbs = MathB.ShapesOfGameObjects(currentPrefabType, settings.pillar.radius, (int) settings.pillar.amount,settings.pillar.shape);

        //Organize pillars nicely in this folder under this transform
        folder = new GameObject("Pillars-" + bulbs.Count);
        folder.transform.SetParent(transform);

		for (int i = 0; i < bulbs.Count; i++) //needs to be <= sample rate or error
		{
			bulbs[i].transform.SetParent(folder.transform);
        }
        isBuilding = false;
    }
    ...
    void Update()
    {
        ...
        for (int i = 0; i < bulbs.Count; i++) //needs to be <= sample rate or error
        {
			...
			bulbs[i].GetComponent<Pillar>().setIntensity(level);
			...
			if (level / previousScale.y > 10) pillars[i].transform.Rotate(Vector3.up, 15);
